...
static inline void send_unreach(struct sk_buff *skb_in, int code, int hook)
{
	...
	if (!skb_in->sk || hook != NF_INET_LOCAL_OUT)
		return;
	...
	if (!sock_owned_by_user(skb_in->sk)) {
		...
		skb_in->sk->sk_err_soft = icmp_err_convert[code].errno;
		pr_debug("ipt_REJECT: sk_err_soft=%d for skb=%p sk=%p\n",
			skb_in->sk->sk_err_soft, skb_in, skb_in->sk);
	}
#endif
}
